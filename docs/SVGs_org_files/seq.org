...
client -> client: getPowerRequired()
client -> client: setPowerCurrent(int s)
client -> ship: updateSection(s.setpowerCurrent(int Value)))
ship -> client: energiekResponse(Ship s)
...
loop
     ...
     client -> ship: findPath(Section s.getSections, Section start, Section End): List<Section>
     ship -> crew: updateCrewMember(Ship s, List<Section>, CrewMember c)
     crew -> ship: applyCrewSkills(Ship s)
     ship -> client: crewMemberController.response(Ship s)
...
client -> ship: startAttack(Weapon w, Section sec, Ship s)
    loop
         ship -> ship: getShip(ship): Ship
         ship -> weapon: ship.getShield(): Shield
         weapon -> weapon: canAttack(Weapon W, Shield): Boolean
         weapon -> weapon: calculateDammage(Weapon w , Section s)
         weapon -> client: applyAttack(Weapon w, Weapon): Weapon // HP, Shield
         client -> ship: verifyResponse(Ship sNew): Boolean
    end loop
    ship -> client: updateShip(Ship s)
