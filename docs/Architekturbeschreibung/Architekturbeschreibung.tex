...
\section{Ausführungssicht} \label{sec:ausfuehrung}
 ...
 In diesem Absatz wird Ausführungssicht des Systems beschrieben. Das enthält die Kommunikation bzw. die Interaktion der Komponente und auch deren Hostrechner, die die Komponenten darauf laufen. \\Die untenstehende Abbildung dient als Ausführungssicht für das Spiel $ " $Even Geiler Than Geiler Than Faster Than Light$ "$ . Der Spiel wird mit dem Client-Server Architektur Pattern realisiert.\\
 \begin{figure}[!ht]
 	\centering
 	\includegraphics[width=0.85\textwidth]{Ausfuehrungsicht_Diagram.png}
 	\caption{Ausführungssicht}
 	\label{fig2}

 \end{figure}
 	Hier kommt die Abbildung
 \\Die Abbildung besteht aus zwei geteilte Box. Der oberste Teil spielgelt der Server und er enthält mehrere Module sowie Service, Datenbank(H2), Business Logic, Communication und DAO(Date Access Object). Der Server wird durch TCP mit dem Client kommuniziert und es ist möglich, dass der Server ein bis zwei Client bzw. Spieler gleichzeitig kommunizieren kann. Der Server stell die Verbindung mit Datenbank sicher. In dem er über Service interagieren kann. Dadurch, dass wir eine Datenbank benötigten, wird die Datenbank mit hilfe von DAO-Modul persistiert und verwaltet. Die ausgewählte leichgewichte Datenbank(H2) wird mit dem Anwendungsserver auf den selben Gerät laufen. 
 \\
 Unterester Teil dient als Client und besteht aus drei Module sowie Model, Communication und GUI. Der Client wird duch TCP der Server ansprechen. 
